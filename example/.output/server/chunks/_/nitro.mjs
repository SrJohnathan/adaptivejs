...
function parseRoutePathServer(filePath) {
...
function matchRouteServer(routePath, pathname) {
...
function isClientBoundaryTag(tag) {
...
function renderToStringWithMetadata(node) {
  var _a;
  const context = { clientModuleIds: /* @__PURE__ */ new Set() };
  return {
    html: renderNode(node, context),
    clientModuleIds: Array.from((_a = context.clientModuleIds) != null ? _a : [])
  };
}
function renderNode(node, context) {
  ...
  if (typeof node.tag === "function") {
    const result = node.tag({
      ...(_a = node.props) != null ? _a : {},
      children: (_b = node.children) != null ? _b : []
    });
    return renderNode(result, context);
  }
  if (node.tag === "Fragment") {
    return renderNode((_c = node.children) != null ? _c : [], context);
  }
...
function renderClientBoundary(node, context) {
  ...
  const moduleId = String((_b = props["data-adaptive-client-module"]) != null ? _b : "");
  const exportName = String((_c = props["data-adaptive-client-export"]) != null ? _c : "default");
  const rawProps = String((_d = props["data-adaptive-client-props"]) != null ? _d : "{}");
  if (moduleId) {
    (_e = context.clientModuleIds) == null ? void 0 : _e.add(moduleId);
  }
  const innerHtml = renderNode((_f = node.children) != null ? _f : [], context);
...
async function createRouter(url, routes = [], options) {
  var _a, _b;
  const isProduction = (_a = options == null ? void 0 : options.isProduction) != null ? _a : true;
  const sourceDir = (options == null ? void 0 : options.sourceDir) || path.join(process.cwd(), "src");
  const serverBuildDir = (options == null ? void 0 : options.serverBuildDir) || path.join(process.cwd(), "dist", "server");
  const clientBuildDir = (options == null ? void 0 : options.clientBuildDir) || path.join(process.cwd(), "dist", "client");
  ...
  return {
    html: rendered.html,
    params: (_b = routeMatch.params) != null ? _b : {},
    query: uri.query,
    clientEntries: Array.from(new Set([routeMatch.clientEntry ? clientManifest[routeMatch.clientEntry] : null, ...rendered.clientModuleIds.map((moduleId) => {
      var _a2;
      return (_a2 = clientManifest[moduleId]) != null ? _a2 : null;
...
const _DAziuY = eventHandler(async (event) => {
...
const handlers = [
  { route: '/**', handler: _DAziuY, lazy: false, middleware: false, method: undefined }
];
