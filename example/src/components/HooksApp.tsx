...
import { savePreferences, subscribe } from "../actions";
...
export default function HooksApp() {
  ...
  const callPreferencesSave = async () => {
    const result = await savePreferences({
      email: email(),
      count: count(),
      subscribed: true
    });
    setServerMessage(`Preferences: ${result.summary}`);
  };
  ...
  return (
    <CounterContext.Provider value={{ label: "shared through createContext()" }}>
      <section className="hero">
        ...
        <input
          value={email()}
          onInput={(event: Event & { target: HTMLInputElement }) => setEmail(event.target.value)}
        ...
        />
        <button type="button" onClick={callSubscribe}>
          Call server action
        </button>
        <button type="button" onClick={callPreferencesSave}>
          Save preferences
        </button>
        <div className="grid">
          ...
          <article className="card">
            ...
            <p className="muted">Signal-style naming</p>
          </article>
        </div>
      </section>
