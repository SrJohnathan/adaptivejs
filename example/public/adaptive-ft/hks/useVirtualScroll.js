...
export function useVirtual(options) {
	const { size, parentRef, estimateSize = () => 50, overscan = 3, paddingStart = 0, paddingEnd = 0, horizontal = false, scrollToFn, onScrollStart, onScrollEnd } = options;
	...
	const getVirtualItems = useCallback(() => {
		...
		const endOffset = Math.min(positionsRef.current?.totalSize || 0, scrollOffset() + viewportSize + overscan * (estimateSize(0) || 50));
		const virtualItems = [];
	...
	const getVirtualItemForOffset = useCallback((offset) => {
		const { itemPositions } = positionsRef.current;
	...
	const getOffsetForAlignment = useCallback((index, align) => {
		const { itemPositions } = positionsRef.current;
