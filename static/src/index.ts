...
function getAdaptiveRoot(appDir: string) {
  return path.join(appDir, ".adaptivejs");
}

function getAdaptiveOutputDir(appDir: string, preset: string) {
  return path.join(getAdaptiveRoot(appDir), "output", preset);
}

function getAdaptiveAdapterDir(appDir: string, preset: string) {
  return path.join(getAdaptiveRoot(appDir), "adapters", "nitro", preset);
}

function getAdaptiveNitroCacheDir(appDir: string, preset: string) {
  ...
  return createApp({
    ...
    routers: [
      ...
      },
      {
        ...
        type: "http",
        handler: path.join(
          getAdaptiveAdapterDir(appDir, requestedPreset),
          "adaptive-handler.js",
        ),
        target: "server",
      },
...
export async function buildAdaptive(options: StaticOptions = {}) {
  ...
  const outputDir =
    options.outputDir || getAdaptiveOutputDir(appDir, requestedPreset);
  ...
  const bundledAdaptivePackagesDir = path.join(
    appDir,
    "node_modules",
    "@adaptivejs",
  );
  const deployedAdaptivePackagesDir = path.join(
    deployedServerDir,
    "node_modules",
    "@adaptivejs",
  );
...
`;
  ...
  await copyDirContents(
    bundledAdaptivePackagesDir,
    deployedAdaptivePackagesDir,
    {
      includeTopLevel: new Set(["common", "core", "ft", "ssr", "ui", "web"]),
    },
  );

  if (requestedPreset === "netlify" || requestedPreset === "vercel") {
    await fs.mkdir(outputDir, { recursive: true });
    await copyDirContents(path.join(appDir, "public"), outputDir, {
      skip: new Set(["_redirects"]),
    });
  ...
  if (requestedPreset === "static") {
    console.warn(
        "Warning: preset 'static' needs prerender routes to generate full static HTML. For SSR deploy, use preset 'vercel', 'netlify' or 'node'.",
    );
  }
}
...
async function copyDirContents(
  fromDir: string,
  toDir: string,
  options: { skip?: Set<string>; includeTopLevel?: Set<string>; depth?: number } = {},
) {
  try {
...
export async function buildStatic(options: StaticOptions = {}) {
  return buildAdaptive({ ...options, preset: "static" });
}
...
export async function previewAdaptive(options: PreviewOptions = {}) {
  ...
  const staticRoot =
    preset === "netlify" ? outputDir : path.join(appDir, "dist");
  const runtimeClientRoot = path.join(
    getAdaptiveAdapterDir(appDir, preset),
    "runtime",
    "client",
  );
  const handlerModulePath =
    preset === "netlify"
      ? path.join(getAdaptiveOutputDir(appDir, "netlify"), "server", "main.mjs")
      : path.join(
          getAdaptiveOutputDir(appDir, "vercel"),
          "functions",
          "__fallback.func",
          "index.mjs",
        );
  ...
  if (preset === "netlify") {
    process.env.ADAPTIVE_RUNTIME_ROOT = path.join(
      getAdaptiveOutputDir(appDir, "netlify"),
      "server",
      "adaptive-runtime",
    );
