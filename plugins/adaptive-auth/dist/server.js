...
export function createAdaptiveAuth(options = {}) {
	const resolvedOptions = {
		...
		store: options.store ?? new MemoryAuthSessionStore(),
	...
	const auth = {
		...
		async createSession(input) {
			...
			const record = {
				...
				issuedAt,
				expiresAt: input.tokens.expiresAt ?? issuedAt + resolvedOptions.cookieMaxAge * 1e3
			};
...
export function createAuthPlugins(auth) {
	return [
		{
			...
			callback: async (req, res) => {
				...
				if (!provider) {
					res.status(404).json({
						success: false,
						error: `Unknown auth provider '${providerName ?? "unknown"}'.`
					});
					return;
				}
				if (!provider.getAuthorizationUrl) {
					res.status(405).json({
						success: false,
						error: `Provider '${provider.name}' does not support redirect login.`
					});
					return;
				}
		...
		},
		{
			...
			callback: async (req, res) => {
				...
				if (!provider.authenticateCredentials) {
					res.status(405).json({
						success: false,
						error: `Provider '${provider.name}' does not support credential login.`
					});
					return;
				}
				try {
		...
		},
		{
			...
			callback: async (req, res) => {
				...
				if (!provider.exchangeCode) {
					res.status(405).json({
						success: false,
						error: `Provider '${provider.name}' does not support OAuth callback.`
					});
					return;
				}
...
export async function resolveAuthSession(auth, request) {
	return auth.getPublicSession(request);
}
export async function requireAuthSession(auth, request) {
...
async function handleProviderFailure(auth, res, message) {
...
function getRequestOrigin(request) {
	...
	const protocol = typeof forwardedProto === "string" && forwardedProto.length > 0 ? forwardedProto.split(",")[0] : "http";
...
async function readCredentialPayload(request) {
