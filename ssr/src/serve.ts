...
export type RouteDefinition = {
  path: string;
  component: (props?: { params?: Record<string, string>; querys?: Record<string, string> }) => any;
  clientEntry?: string;
  metadata?: AdaptiveMetadataResolver;
};

export async function init_server(options: ServerOptions = {}): Promise<Express> {
  const isAdaptiveDev = process.env.ADAPTIVE_DEV === "true";
  const isProduction = process.env.NODE_ENV === "production";
  const app = express();
  const requestedPort = normalizePort(options.port || process.env.PORT || 3000);
  ...
  app.use(async (req, res) => {
    try {
      ...
      const routeContext: AdaptiveMetadataContext = {
        url,
      ...
      const html = applyAssetVersion(
        injectIntoTemplate(template, result.html, `${hydrationScript}${devReloadScript}`, result.clientEntries ?? [], headHtml),
        assetVersion,
      );
      if (isAdaptiveDev) {
...
export async function createRouter(
  ...
  const rendered = renderToStringWithMetadata({
    ...
    props: { params: routeMatch.params, querys: uri.query },
    children: []
  });
  ...
  return {
    ...
    clientEntries: Array.from(
      new Set(
        [
          routeMatch.clientEntry ? clientManifest[routeMatch.clientEntry] : null,
          ...rendered.clientModuleIds.map((moduleId: string) => clientManifest[moduleId] ?? null)
...
async function handleActionRequest(
  ...
  try {
    ...
    if (typeof fn !== "function") {
      res.status(404).json({ success: false, error: `Action '${actionRequest.actionName}' not found.` });
      return;
    }
...
async function parseActionRequest(req: Request, defaultModuleId?: string) {
  ...
  if (typeof contentType === "string" && contentType.includes("multipart/form-data")) {
    ...
    return await new Promise<{ moduleId: string; actionName: string; args: any[] }>((resolve, reject) => {
      form.parse(req, async (error: any, fields: any, files: any) => {
        ...
        try {
          ...
          const merged = { ...fields, ...normalizedFiles };
          ...
          const args = headerArgsMode === "formdata-single"
            ? [createNodeFormData(fields, normalizedFiles)]
            : Array.isArray(payload.args)
              ? payload.args
              : payload.args !== undefined
                ? [payload.args]
                : [];
          resolve({ moduleId, actionName, args });
        } catch (parseError) {
          reject(parseError);
        }
      });
...
function createNodeFormData(fields: Record<string, any>, files: Record<string, any>) {
...
function appendFormDataEntries(formData: FormData, source: Record<string, any>) {
  for (const [key, value] of Object.entries(source)) {
    appendFormDataValue(formData, key, value);
  }
}

function appendFormDataValue(formData: FormData, key: string, value: any) {
...
function registerPlugin(app: Express, base: string, plugin: Plugin) {
  const fullPath = `${base}${plugin.path}`;
  switch (plugin.method) {
    case "GET":
      app.get(fullPath, (req, res) => plugin.callback(req as AdaptiveRequest, res as AdaptiveResponse));
      break;
    case "POST":
      app.post(fullPath, (req, res) => plugin.callback(req as AdaptiveRequest, res as AdaptiveResponse));
      break;
    case "PUT":
      app.put(fullPath, (req, res) => plugin.callback(req as AdaptiveRequest, res as AdaptiveResponse));
      break;
    case "DELETE":
      app.delete(fullPath, (req, res) => plugin.callback(req as AdaptiveRequest, res as AdaptiveResponse));
      break;
    case "PATCH":
      app.patch(fullPath, (req, res) => plugin.callback(req as AdaptiveRequest, res as AdaptiveResponse));
      break;
  }
}
...
async function loadRenderModule(options: { isProduction: boolean; sourceDir: string; serverBuildDir: string }) {
  const modulePath = options.isProduction
    ? path.join(options.serverBuildDir, "layout.js")
    : path.join(options.sourceDir, "layout.ts");
  return import(pathToFileURL(modulePath).href);
}

function injectIntoTemplate(template: string, html: string, hydrationScript: string, clientEntries: string[], headHtml = "") {
...
async function loadBuildVersion(clientBuildDir: string) {
  try {
    const metadata = await fs.readFile(path.join(clientBuildDir, "build-meta.json"), "utf8");
    const parsed = JSON.parse(metadata) as { buildId?: string };
...
async function loadServerModuleManifest(serverBuildDir: string) {
  try {
...
function mergeMetadata(
  base: AdaptiveMetadata | null,
  override: AdaptiveMetadata | null
...
function renderMetadataTags(metadata: AdaptiveMetadata | null) {
  if (!metadata) {
    return "";
  }
...
function normalizeServerModuleId(moduleId: string) {
...
function readActionMeta(value: any) {
  if (Array.isArray(value)) {
...
function readActionHeader(value: string | string[] | undefined) {
  return Array.isArray(value) ? value[0] : value;
}

function normalizeIndexedStructures(value: any): any {
  if (Array.isArray(value)) {
  ...
  if (isIndexedObject) {
    return keys
      ...
      .map((key) => normalizeIndexedStructures(value[key]));
  }
...
function createPrecompressedMiddleware(rootDir: string) {
...
function createDevReloadScript(base: string) {
...
function createStaticOptions(isAdaptiveDev: boolean) {
  return {
    index: false,
    etag: !isAdaptiveDev,
...
function createAssetBase(base: string) {
...
async function listenWithPortFallback(app: Express, startPort: number) {
  let port = startPort;
...
async function tryListen(app: Express, port: number) {
