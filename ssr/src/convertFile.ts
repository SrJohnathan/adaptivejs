...
export async function convertFile(formidableFile: FormidableFile): Promise<File> {
...
function getFileName(filePath: string):Promise<Buffer<ArrayBufferLike>> {
...
export function formDataToObject(formData: Record<string, any>): any {
    const result: any = {};
...
export async function normalizeFormidableFiles(obj: Record<string, any>):  Promise< Record<string, any>> {
    ...
    function isFileObject(file: any): boolean {
        return (
            file &&
            typeof file === 'object' &&
            'filepath' in file &&
