import { CLIENT_BOUNDARY_END, CLIENT_BOUNDARY_MODE_CLIENT, CLIENT_BOUNDARY_MODE_HYDRATE, CLIENT_BOUNDARY_START_PREFIX, REACTIVE_CHILD_END, REACTIVE_CHILD_START, REACTIVE_STRUCT_START, REACTIVE_STRUCT_END, REACTIVE_LIST_START, REACTIVE_LIST_END, REACTIVE_ASYNC_START, REACTIVE_ASYNC_END, HYDRATE_SLOT_END, HYDRATE_SLOT_START, isClientBoundaryTag, isHydrateSlotTag } from "./client-boundary.js";
import { HYDRATION_BOUNDARY_ID_ATTR, HYDRATION_MANIFEST_ATTR } from "./hydration-manifest.js";
...
function renderNode(node, context) {
	if (typeof node === "function") {
		const preview = node();
		const reactiveId = context.hydrateReactiveCounter ? String(context.hydrateReactiveCounter.value++) : null;
...
function resolveComponentProps(node) {
	const props = { ...node.props ?? {} };
	const children = getVNodeChildren(node);
	if (Array.isArray(children) && children.length > 0 || !Array.isArray(children) && children !== undefined && children !== null || props.children === undefined) {
...
function renderClientBoundary(node, context) {
	...
	const manifestHtml = mode === CLIENT_BOUNDARY_MODE_HYDRATE ? renderHydrationManifest(boundaryId, boundaryContext.hydrateManifest ?? []) : "";
...
function renderHydrateSlot(node, context) {
...
function renderHydrationManifest(boundaryId, instructions) {
...
function appendHydrationManifestForProps(props, id, context) {
	...
	for (const [key, value] of Object.entries(props)) {
		...
		if (key !== "children" && key !== "ref" && typeof value === "function" && isSupportedDynamicHydrationPropName(key)) {
			context.hydrateManifest.push({
...
function nextHydrationInstructionKey(context, kind) {
...
function resolveHydrationInstructionKey(kind, index) {
	switch (kind) {
		case "event": return `e${index}`;
		case "ref": return `f${index}`;
		case "reactive": return `rx${index}`;
		case "dynamicProp": return `p${index}`;
	}
}
function maybeAllocateHydrationId(props, context) {
...
function resolveReactiveMarkerPair(value) {
...
function resolveManifestReactiveKind(markerStart) {
...
function resolveReactivePreviewForSSR(value) {
	if (isPromiseLike(value)) {
		return "";
	}
	return value;
