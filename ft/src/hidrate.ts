import { useEffect, useLayoutEffect } from "./state.js";
...
export function renderToDOM(vNode: any): Node {
  ...
  if (typeof vNode.tag === "function") {
    return renderToDOM(vNode.tag({ ...(vNode.props ?? {}), children: vNode.children ?? [] }));
  }
...
function hydrateChildren(parent: Node, vChildren: any[], path: string) {
  let cursor = 0;
...
function hydrateNode(parent: Node, input: any, index: number, path: string): number {
  ...
  if (typeof vNode.tag === "function") {
    return hydrateNode(parent, vNode.tag({ ...(vNode.props ?? {}), children: vNode.children ?? [] }), index, `${path}#component`);
  }
