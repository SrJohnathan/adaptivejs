...
import { createElement } from "@adaptive-js/jsx";
import {
  ...
  findMatchingMarkerEnd, debugHydrationMarkers
} from "./hidrate.js";
  ...
  commentBoundaries.forEach((boundary) => {
    ...
    if (boundary.hasChildren) {
      props.children = boundary.mode === "hydrate"
          ...
          : collectSiblingNodesBetween(boundary.start, boundary.end);
    }
...
moduleId?: string
  ...
  layoutEffects: Array<{ effect: EffectFn; deps?: DependencyList; ignoredSources?: ReactiveSource[] }>;
  effects: Array<{ effect: EffectFn; deps?: DependencyList; ignoredSources?: ReactiveSource[] }>;
  unsupportedFeatures: string[];
}
{
...
export function cleanupClientComponentScopes(container?: ParentNode) {
  for (const [node, record] of clientBoundaryScopes.entries()) {
    const shouldCleanup = container
        ? !record.isConnected() || record.isWithin(container)
        : !record.isConnected();
