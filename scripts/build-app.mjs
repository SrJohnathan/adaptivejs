...
export { buildDesktopApp } from "./desktop/build-desktop-app.mjs";
...
async function buildTransformedFile(sourcePath, outputPath, cwd, sourceText) {
    ...
    if (result.errors.length > 0) {
        for (const error of result.errors) {
            console.error(
                `${path.relative(rootDir, sourcePath)}: ${error.message}`,
            );
...
async function copyDir(fromDir, toDir, exclude = new Set()) {
    try {
        const entries = await fs.readdir(fromDir, { withFileTypes: true });
        for (const entry of entries) {
...
async function bundleClientEntries({
    ...
    for (const { file, id } of await collectExplicitClientEntries(
        clientSrcDir,
    )) {
...
async function createClientComponentWrappers({ appDir, srcDir, tempDir }) {
...
async function writePageIRArtifacts({ srcDir, serverDistDir, distDir }) {
    ...
    const coreModule = await import(
        pathToFileURL(path.join(rootDir, "core", "dist", "index.js")).href
    );
